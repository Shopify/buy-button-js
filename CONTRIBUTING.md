...
9. (Optional) If you want this new version to immediately take effect on `https://sdks.shopifycdn.com/buy-button/latest/buybutton.js`, you can purge the CDN cache via [Infra Central](https://infra-central.shopify.io/edge/purges):
...
It's rare that you would ever want to rollback the `latest` CDN version without releasing a new version on npm - typically the `latest` npm version and `latest` CDN version are kept in sync. But if needed, follow these steps to roll back the `latest` CDN version without publishing a normal npm release.
...
> [!NOTE]
> The release workflow detects the `ROLLBACK` marker in `package.json` and automatically skips `npm publish` — no version is published to npm. The CDN revert is handled entirely by the Shipit deploy and CDN purge below; npm is not involved in rollbacks.
